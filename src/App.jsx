...
const SIZES = [
  { label:"1:1",  w:1024, h:1024, use:"Instagram Square", api:"square_1_1"       },
  { label:"4:5",  w:896,  h:1120, use:"Instagram Feed",   api:"social_story_4_5"  },
  { label:"9:16", w:768,  h:1344, use:"Stories / Reels",  api:"social_story_9_16" },
  { label:"16:9", w:1344, h:768,  use:"YouTube/LinkedIn", api:"widescreen_16_9"   },
  { label:"3:4",  w:864,  h:1152, use:"Portrait Print",   api:"classic_3_4"       },
  { label:"4:3",  w:1152, h:864,  use:"Presentation",     api:"classic_4_3"       },
];

const STYLES = [
  { label:"Cinematic",      api:"cinematic"    },
  { label:"Photorealistic", api:"photographic" },
  { label:"Digital Art",    api:"digital_art"  },
  { label:"Fantasy Art",    api:"fantasy_art"  },
  { label:"3D Model",       api:"3d_model"     },
  { label:"Anime",          api:"anime"        },
  { label:"Neon Punk",      api:"neon_punk"    },
  { label:"Line Art",       api:"line_art"     },
];

async function generateImage(prompt, negPrompt, sizeApi, styleApi) {
  const res = await fetch("/api/generate", {
    method:"POST",
    headers:{"Content-Type":"application/json"},
    body: JSON.stringify({ prompt, negPrompt, sizeApi, styleApi }),
  });
...
export default function App() {
  ...
  const [negPrompt,  setNegPrompt]  = useState("AI artifacts, plastic skin, oversaturation, watermark, blurry, low quality");
  ...
  const handleGenerate = async () => {
    ...
    setGenerating(true); setError(null); setImgSrc(null); setElapsed(0);
    const t0 = Date.now();
    const timer = setInterval(() => setElapsed(Math.floor((Date.now()-t0)/1000)), 500);
    const cs = SIZES.find(s => s.label === size);
    const st = STYLES.find(s => s.label === style);
    try {
      const src = await generateImage(prompt, negPrompt, cs.api, st.api);
      setImgSrc(src);
      setHistory(prev => [{
        id: Date.now(),
  ...
  const handleDownload = () => {
    ...
    a.href = imgSrc; a.download = `nanaban-${Date.now()}.jpg`; a.click();
  };
  ...
  const CSS = `
    ...
    *{box-sizing:border-box;margin:0;padding:0;}
    ...
    .navbar{display:flex;align-items:center;justify-content:space-between;padding:12px 24px;border-bottom:1px solid rgba(255,255,255,.06);background:rgba(7,8,10,.96);backdrop-filter:blur(20px);position:sticky;top:0;z-index:100;}
    ...
    <div style={{minHeight:"100vh",background:"#07080A",fontFamily:"'DM Sans',sans-serif",display:"flex",flexDirection:"column"}}>
      ...
      <nav className="navbar">
        <div style={{display:"flex",alignItems:"center",gap:9}}>
          ...
          <span className="badge" style={{background:"rgba(249,171,0,.1)",color:"#F9AB00",border:"1px solid rgba(249,171,0,.2)",marginLeft:4}}>Classic Fast · 1 credit/img</span>
        </div>
        <div className="ntabs">
        ...
        <span className="badge" style={{background:"rgba(52,168,83,.1)",color:"#34A853",border:"1px solid rgba(52,168,83,.2)"}}>✓ Freepik AI</span>
      </nav>
          ...
          <div className="lpanel">
            ...
            <div>
              ...
              <div className="schips">
                {STYLES.map(s=><div key={s.label} className={`schip ${style===s.label?"on":""}`} onClick={()=>setStyle(s.label)}>{s.label}</div>)}
              </div>
            </div>
            ...
            <button className={`genbtn ${generating?"busy":"idle"}`} onClick={handleGenerate} disabled={generating||!prompt.trim()}>
              {generating
                ? <><div className="spin"/>Generating... {elapsed}s</>
                : <>✦ Generate Image</>}
            </button>
          </div>
          ...
          <div style={{flex:1,display:"flex",flexDirection:"column"}}>
              ...
              <div className="imgbox" style={{width:dw,height:dh}}>
                ...
                {generating && (
                  <div className="overlay">
                    ...
                    <div style={{fontSize:12,color:"rgba(255,255,255,.4)",marginTop:10,fontFamily:"'DM Sans',sans-serif"}}>Classic Fast generating...</div>
                    <div style={{fontSize:11,color:"rgba(255,255,255,.22)",marginTop:4,fontFamily:"'DM Sans',sans-serif"}}>{elapsed}s elapsed</div>
                  </div>
                )}
                {!imgSrc && !generating && (
            ...
            <div className="metabar">
              <div style={{display:"flex",gap:20}}>
                {[["Model","Classic Fast"],["Size",`${cs.label} · ${cs.w}×${cs.h}`],["Style",style],["Cost","1 credit/image"]].map(([k,v])=>(
                  <div key={k}>
