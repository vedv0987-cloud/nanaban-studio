...
// ── Image Generation via Gemini 2.0 Flash (browser-safe) ──
async function generateImage(apiKey, prompt, style, negPrompt) {
  ...
  if (!res.ok) {
    ...
    if (msg.includes("API_KEY") || msg.includes("key")) throw new Error("Invalid API key — check aistudio.google.com");
    if (msg.includes("quota")) throw new Error("Quota exceeded — try again in a minute");
    throw new Error(msg);
  }
  ...
  if (!imgPart) {
    ...
    throw new Error(textPart?.text || "No image returned — try a different prompt");
  }
...
export default function App() {
  ...
  const saveKey = (k) => {
    setApiKey(k);
    try { localStorage.setItem("nb_gemini_key", k); } catch {}
  };

  const handleGenerate = async () => {
    if (!prompt.trim()) return;
    if (!apiKey.trim()) {
      setError("Add your free Gemini API key first → aistudio.google.com/app/apikey");
      return;
    }
    setGenerating(true); setError(null); setImgSrc(null);
    try {
      const src = await generateImage(apiKey, prompt, style, negPrompt);
      setImgSrc(src);
      setHistory(prev => [{
        id:Date.now(), prompt:prompt.slice(0,60)+(prompt.length>60?"...":""),
        size, style, src, time:new Date().toLocaleTimeString()
      }, ...prev.slice(0,15)]);
    } catch(e) {
      setError(e.message);
  ...
  const handleDownload = () => {
    ...
    a.href = imgSrc; a.download = `nanaban-${Date.now()}.png`; a.click();
  };
  ...
  const CSS = `
    ...
    *{box-sizing:border-box;margin:0;padding:0;}
    ...
    .signout:hover{color:rgba(255,255,255,.6);background:rgba(255,255,255,.07);}
    /* Left panel */
    ...
    /* API key */
    .key-wrap{background:rgba(66,133,244,.06);border:1px solid rgba(66,133,244,.15);border-radius:12px;padding:12px;}
    ...
    .overlay{position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;background:rgba(7,8,10,.75);backdrop-filter:blur(6px);border-radius:16px;}
    ...
    .prog-bar{height:100%;background:linear-gradient(90deg,#F9AB00,#34A853);border-radius:99px;animation:progbar 18s ease-out forwards;}
    /* Meta bar */
      ...
      <div className="card">
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:32}}>
          ...
          <div>
            ...
            <div style={{fontSize:11,color:"rgba(255,255,255,.33)",marginTop:1}}>Powered by Google Gemini</div>
          </div>
        </div>
        <div style={{fontFamily:"'Syne',sans-serif",fontSize:26,fontWeight:800,color:"#fff",lineHeight:1.22,marginBottom:10,letterSpacing:"-0.6px"}}>Real AI image generation</div>
        <div style={{fontSize:14,color:"rgba(255,255,255,.38)",marginBottom:28,lineHeight:1.65}}>Sign in with Google, then connect your free Gemini API key to generate real images instantly.</div>
        ...
        <div className="ibox" style={{marginTop:16}}>
          <strong>Step 2:</strong> Get your <strong>free</strong> Gemini API key →{" "}
          ...
          <br/>No billing required. Free tier: 15 images/min.
        </div>
      </div>
    </div>
  );
    ...
    <div style={{minHeight:"100vh",background:"#07080A",fontFamily:"'DM Sans',sans-serif",display:"flex",flexDirection:"column"}}>
      ...
      <nav className="navbar">
        ...
        <div style={{display:"flex",alignItems:"center"}}>
          <div className="pill">
            {user.picture
              ? <img className="av" src={user.picture} alt="" referrerPolicy="no-referrer"/>
          ...
          <div className="lpanel">
            ...
            <div className="key-wrap">
              ...
              <div style={{fontSize:11,color:"rgba(255,255,255,.38)",lineHeight:1.5}}>
                {apiKey
                  ? <span style={{color:"#34A853"}}>✓ Key connected — ready to generate</span>
                  : <>Get free key → <a className="ilink" href="https://aistudio.google.com/app/apikey" target="_blank" rel="noreferrer">aistudio.google.com/app/apikey</a></>}
              </div>
            ...
            <div>
              ...
              <div className="schips">
                {STYLES.map(s=>(
                  <div key={s} className={`schip ${style===s?"on":""}`} onClick={()=>setStyle(s)}>{s}</div>
                ))}
              </div>
            </div>
            ...
            <button
              className={`genbtn ${generating?"busy":"idle"}`}
              onClick={handleGenerate}
              disabled={generating || !prompt.trim()}
            >
              {generating
                ? <><div className="spin"/>Generating image...</>
                : <>✦ Generate Image</>}
            </button>
          </div>
              ...
              <div className="imgbox" style={{width:dw,height:dh}}>
                ...
                {generating && (
                  <div className="overlay">
                    ...
                    <div style={{fontSize:12,color:"rgba(255,255,255,.4)",marginTop:10,fontFamily:"'DM Sans',sans-serif"}}>Generating real image...</div>
                  </div>
                )}
                {!imgSrc && !generating && (
